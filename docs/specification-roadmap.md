...
Remaining: `Home.jsx`, `Tab.jsx`, `AdminPanel.jsx` (the three largest/most complex files — all already covered at the golden-path level by the E2E suite, Phase 4), the rest of `ListManager.jsx`'s branches (61.78% today), and `auth.service.js`/`data.service.js` (both thin wrappers around `helpHttp`, lower marginal value). A 100% target may still not be the right call for purely presentational JSX branches in the remaining files.
