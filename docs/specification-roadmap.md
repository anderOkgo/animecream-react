...
**Golden paths covered so far** (3 spec files, all passing against the real stack):
...
**Fixed at the user's explicit request**, in `animecream-data` (not this repo): changed both joins to `LEFT JOIN` in `db-views-procs.sql`, applied to the real local dev DB, and verified two ways — (1) a genre-less row inserted directly now appears in search results with `genre_names: null` (matching how `title_names` already behaves for title-less rows via its pre-existing `LEFT JOIN titles`), and (2) existing multi-genre catalog data (e.g. "Neon Genesis Evangelion") still returns its full, correctly-grouped `genre_names` unchanged. `admin.spec.js` then passed end to end without any workaround needed on the test side. **Not yet deployed to production** — this fix lives only in the local dev DB/repo so far; deploying it goes through `module-data`'s `db-deploy-schema.bat` pipeline (see that repo's `docs/deployment.md`), a separate, explicit decision.
...
2. **`MyLists` E2E** (create/add/remove) — the one golden path from the original Phase 4 plan not yet covered; it's `localStorage`-only (no real backend mutation), so lower risk than admin CRUD was.
...
4. **Deploy the `view_all_info_produtions` genre-visibility fix to production** (see above) — pending explicit go-ahead, since it's a live schema change via `module-data`'s deploy pipeline.
...
- **2026-07-19, later still** — Admin CRUD E2E (`admin.spec.js`, create + update) added at the user's request. Surfaced a real, significant bug: `view_all_info_produtions` (in `animecream-data`) used `INNER JOIN` to genres, silently hiding any series with zero genres from the entire catalog even though `AdminPanel`'s form never required one. Fixed in `animecream-data/schemas/animecream-data/sql/db-views-procs.sql` (→ `LEFT JOIN`) at the user's explicit go-ahead, applied and verified against the real local dev DB (existing multi-genre data unaffected, genre-less rows now correctly appear with `genre_names: null`). All 4 E2E specs pass together. Not yet deployed to production — that's `module-data`'s separate `db-deploy-schema.bat` pipeline, pending its own explicit decision.
